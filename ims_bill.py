...
class Bill_App:
    def __init__(self,root):
        ...
        cname_lbl=Label(F1,bg=bg_color,fg="white",text="Customer Name",font=("times new roman",15,"bold")).grid(row=0,column=0,padx=20,pady=0)
        cname_txt=Entry(F1,textvariable=self.c_name,width=20,font="arial 15",bd=5,relief=SUNKEN).grid(row=0,column=1,pady=4,padx=10)

        cphn_lbl=Label(F1,bg=bg_color,fg="white",text="Phone No.",font=("times new roman",15,"bold")).grid(row=0,column=2,padx=20,pady=5)
        cphn_lbl=Entry(F1,textvariable=self.c_phon,width=20,font="arial 15",bd=5,relief=SUNKEN).grid(row=0,column=3,pady=5,padx=10)

        c_bill_lbl=Label(F1,bg=bg_color,fg="white",text="Bill No.",font=("times new roman",15,"bold")).grid(row=0,column=4,padx=20,pady=5)
        c_bill_lbl=Entry(F1,textvariable=self.search_bill,width=20,font="arial 15",bd=5,relief=SUNKEN).grid(row=0,column=5,pady=5,padx=10)

        bill_btn=Button(F1,text="Search",command=self.find_bill,width=10,bd=5,font="arial 15 bold").grid(row=0,column=6,padx=10,pady=10)

        #============Search Items===============#

        F2=LabelFrame(self.root,bd=10,relief=GROOVE, text="Search Items",font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        F2.place(y=170,width=350,height=370)
        ...
        F3.place(x=340,y=170,width=355,height=370)
        ...
        search_name_btn=Button(F3,command=self.add_item,bg="black",text="Add",fg="white",pady=15,width=11,bd=5,font="arial 15 bold").grid(row=4,column=1,padx=5,pady=5)


        #============Shopping Cart===============#

        F4=LabelFrame(self.root,bd=10,relief=GROOVE, text="Shopping Cart",font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        F4.place(x=680,y=170,width=330,height=370)

        scroll_x=Scrollbar(F4,orient=HORIZONTAL)
        scroll_y=Scrollbar(F4,orient=VERTICAL)
        self.Stock_Table=ttk.Treeview(F4, column=("item_no","item_name","qty","qty/price","price"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.Stock_Table.heading("qty/price",text="perQty")
        ...
        self.txtarea=Text(F5,yscrollcommand=scrol_y.set, xscrollcommand=scrol_x.set)
        ...
        customer_details=Button(F6,bg=bg_color,fg="white",command=self.customer_details,pady=15,width=15,bd=5,text="Customer Details",font="arial 15 bold")
        customer_details.place(x=800,height=85)
        ...
        # btn_F.place(x=0,width=625,height=105)

        total_btn=Button(F6,command=self.find_bill,bg=bg_color,text="Total",fg="White",pady=15,width=11,bd=5,font="arial 15 bold")
        total_btn.place(x=10,width=150, height=85)
        GBill_btn=Button(F6,command=self.bill_area,bg=bg_color,text="Generate Bill",fg="White",pady=15,width=15,bd=5,font="arial 15 bold")
        GBill_btn.place(x=200,width=150, height=85)
        Clear_btn=Button(F6,command=self.clear,bg=bg_color,text="Clear",fg="White",pady=15,width=11,bd=5,font="arial 15 bold")
        Clear_btn.place(x=400,width=150,height=85)
        Exit_btn=Button(F6,command=self.root.destroy,bg=bg_color,text="Exit",fg="White",pady=15,width=11,bd=5,font="arial 15 bold")
        Exit_btn.place(x=600,width=160,height=85)
        self.welcome_bill()
        Print_btn = Button(F6,bg=bg_color,command=self.print_bill,text="Print Bills",fg="white",pady=15,width=11,bd=5,font="arial 15 bold")
        Print_btn.place(x=1030,width=160,height=85)
    ...
    def add_item(self):
        ...
        print(f"item no is {self.item_no.get()}")
        print("list of entries are: " ,self.Stock_Table.get_children())
        for each in listOfEntriesInTreeView:

            if int(self.Stock_Table.item(each)['values'][0])==int(self.item_no.get()):

                print(f"value are {self.Stock_Table}")
                ...
                if new_quantity<available_quantity or new_quantity==available_quantity:
                    ...
                    return
            print('Stock table value: ',each)
        

        if int(self.item_quantity.get())<available_quantity or int(self.item_quantity.get())==available_quantity:
            new_item=(self.item_no.get(),self.item_name.get(),self.item_quantity.get(),self.item_price.get(),self.item_price.get()*self.item_quantity.get())
            self.Stock_Table.insert('',END, values=new_item)
    ...
    def print_bill(self):
        ...
        open(file_receipt,'w').write(self.txtarea.get('1.0',END))
    ...
    def find_bill(self):
        ...
        self.txtarea.insert(END,f"\n=======================================")
        self.txtarea.insert(END,f"\nS.No \tItem Name\t   Qty\t qty/price \tPrice")
        self.txtarea.insert(END,f"\n=======================================")
    ...
    def customer_show(self):
        try:
            con=pymysql.connect(host="localhost",user="root",password="",database="ims")
            cur=con.cursor()
            cur.execute('select * from customer RIGHT JOIN sales_bill ON customer.customer_id=sales_bill.customer_id')
            rows=cur.fetchall()
    ...
    def customer_details(self):
        ...
        self.root2.title("Customer Details")
        self.root2.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')
        self.root2.config(bg='white')
        self.root2.resizable(True,True)
        
        title=Label(self.root2,text='All Customer details',font=('Times New Roman',20),bg='gold',fg='Black',anchor='w',padx=5).pack(fill=X,side=TOP)
        ...
        self.customer_tree=ttk.Treeview(self.root2,columns=("customer_id","customer_name", "customer_phone", "inv_id", "date", "amount"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        self.customer_tree.heading('customer_id',text='Customer id')
        self.customer_tree.heading('customer_name',text='Name')
        self.customer_tree.heading('customer_phone',text='Phone Number')
        self.customer_tree.heading('inv_id',text='Invoice Id')
        self.customer_tree.heading('date',text='Date of bill')
        self.customer_tree.heading('amount',text='amount')
        self.customer_tree["show"]='headings'

        self.customer_tree.column('customer_id',width=100)
        self.customer_tree.column('customer_name',width=100)
        self.customer_tree.column('customer_phone',width=100)
        self.customer_tree.column('inv_id',width=100)
        self.customer_tree.column('date',width=100)
        self.customer_tree.column('amount',width=100)
        scrollx.config(command=self.customer_tree.xview)
        scrolly.config(command=self.customer_tree.yview)
